...
- [x] **3. Local Docker dev environment** — `docker-compose.yml` for Postgres + Redis, `.env.example`. **Done when**: `docker-compose up` gives a working local Postgres reachable from the API. ✅ 2026-07-19 — compose file, `.env.example`, and `docs/local-dev.md` written. ⚠️ **Runtime verification pending**: Docker Desktop is not installed on this machine; run the checks in `docs/local-dev.md` after installing it. Milestone 5 (Prisma) will re-verify Postgres connectivity anyway.
